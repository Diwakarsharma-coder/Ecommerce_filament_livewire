        ...
        <div class="flex flex-wrap mb-24 -mx-3">
          <div class="w-full pr-2 lg:w-1/4 lg:block">
                ...
                @foreach ($categories as $category)
                    <li class="mb-4" wire:key="{{ $category->id }}">
                        <label for="{{ $category->slug }}" class="flex items-center dark:text-gray-400 cursor-pointer">
                            <input id="{{ $category->slug }}" wire:model.live="selectedCategory" value="{{ $category->id }}" type="checkbox" class="w-4 h-4 mr-2">
              ...
              <ul>
                @foreach ($brands as $brand)
                    <li class="mb-4" wire:key="{{ $brand->id }}" >
                        <label for="{{ $brand->slug }}" class="flex items-center dark:text-gray-300 cursor-pointer">
                            <input id="{{ $brand->slug }}" wire:model.live="selectedBrand" value="{{ $brand->id }}" type="checkbox" class="w-4 h-4 mr-2">
              ...
              <ul>
                <li class="mb-4">
                  <label for="is_featured" class="flex items-center dark:text-gray-300">
                    <input id="is_featured" wire:model.live="is_featured" type="checkbox" value="1" class="w-4 h-4 mr-2">
            ...
            <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
              ...
              <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
              <div>
                <p>{{ Number::Currency($price_range, 'INR') }}</p>
                ...
                <div class="flex justify-between ">
                  <span class="inline-block text-lg font-bold text-blue-400 ">{{ Number::Currency(100000, 'INR') }}</span>
            ...
            <div class="px-3 mb-4">
              <div class="items-center justify-between hidden px-3 py-2 bg-gray-100 md:flex dark:bg-gray-900 ">
                <div class="flex items-center justify-between">
                  <select wire:model.live="sort_by" class="block w-40 text-base bg-gray-100 cursor-pointer dark:text-gray-400 dark:bg-gray-900">
              ...
              @foreach ($products as $product)
                <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key="{{ $product->id }}">
                    ...
                    <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700">
                        ...
                        </svg><span wire:loading.remove wire:target="addToCart({{ $product->id }})">Add to Cart</span> <span wire:loading wire:target="addToCart({{ $product->id }})" class="text-red-500">Adding....</span>
                        </a>
